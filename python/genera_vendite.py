...
TARGET_MEAN_DAILY_TOTAL = 320
WEIBULL_BETA = 1.7
UNIT_PRICE_MULTIPLIER = 0.6
...
def build_connection_strings(database=None):
    database_part = f'DATABASE={database};' if database else ''
    connection_strings = []
    for server in SERVER_CANDIDATES:
        connection_strings.extend([
            f'DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};{database_part}Trusted_Connection=yes;Encrypt=yes;TrustServerCertificate=yes',
            f'DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};{database_part}Trusted_Connection=yes;Encrypt=no',
            f'DRIVER={{ODBC Driver 17 for SQL Server}};SERVER=tcp:{server},1433;{database_part}Trusted_Connection=yes;Encrypt=no',
            f'DRIVER={{SQL Server}};SERVER={server};{database_part}Trusted_Connection=yes',
        ])
    return connection_strings


connection_string_test_fallbacks = build_connection_strings()
connection_string_fallbacks = build_connection_strings(DATABASE)
...
def connect_with_fallback(connection_strings, autocommit=True):
    ...
    for candidate in connection_strings:
        try:
            ...
            server = next(
                (part.replace("SERVER=", "") for part in candidate.split(";") if part.startswith("SERVER=")),
                SERVER,
            )
            print(f"Tentativo connessione con {driver} verso {server}...")
...
""".format(SAMPLE_START_DATE=SAMPLE_START_DATE)
    ...
    historical_mean_daily_total = daily_totals.mean()
    mean_daily_total = TARGET_MEAN_DAILY_TOTAL
    ...
    query_special_offers = "SELECT * FROM project.specialOffer_prj"
    df_special_offers = pd.read_sql(query_special_offers, conn)
    if df_special_offers.empty:
        raise ValueError("project.specialOffer_prj non contiene fasce di sconto utilizzabili.")

    special_offer_column_map = {column.lower(): column for column in df_special_offers.columns}
    required_special_offer_columns = {
        'specialofferid': 'SpecialOfferID',
        'minqty': 'MinQty',
        'maxqty': 'MaxQty',
    }
    for source_name, target_name in required_special_offer_columns.items():
        if source_name not in special_offer_column_map:
            raise ValueError(f"project.specialOffer_prj non contiene la colonna richiesta {target_name}.")
        df_special_offers = df_special_offers.rename(columns={special_offer_column_map[source_name]: target_name})

    if 'unitpricediscount' in special_offer_column_map:
        df_special_offers = df_special_offers.rename(
            columns={special_offer_column_map['unitpricediscount']: 'UnitPriceDiscount'}
        )
    elif 'discountpct' in special_offer_column_map:
        df_special_offers = df_special_offers.rename(
            columns={special_offer_column_map['discountpct']: 'UnitPriceDiscount'}
        )
    else:
        raise ValueError(
            "project.specialOffer_prj deve contenere UnitPriceDiscount oppure DiscountPct "
            "per valorizzare daily_orderdetails_temp."
        )

    df_special_offers = df_special_offers[
        ['SpecialOfferID', 'MinQty', 'MaxQty', 'UnitPriceDiscount']
    ].copy()
    df_special_offers['SpecialOfferID'] = df_special_offers['SpecialOfferID'].astype(int)
    df_special_offers['MinQty'] = pd.to_numeric(df_special_offers['MinQty'], errors='coerce').fillna(0).astype(int)
    df_special_offers['MaxQty'] = pd.to_numeric(df_special_offers['MaxQty'], errors='coerce')
    df_special_offers['UnitPriceDiscount'] = pd.to_numeric(
        df_special_offers['UnitPriceDiscount'],
        errors='coerce'
    ).fillna(0.0).astype(float)
    df_special_offers = df_special_offers.sort_values(
        ['MinQty', 'MaxQty', 'UnitPriceDiscount'],
        ascending=[False, True, False],
        na_position='last'
    ).reset_index(drop=True)

    discount_special_offer_ranges = df_special_offers[
        df_special_offers['UnitPriceDiscount'] > 0
    ].copy()
    historical_offer_qty_ranges = []
    for _, offer_row in discount_special_offer_ranges.iterrows():
        min_qty = max(1, int(offer_row['MinQty']))
        max_qty_value = offer_row['MaxQty']
        max_qty = None if pd.isna(max_qty_value) else max(min_qty, int(max_qty_value))
        historical_range_mask = (
            (df_daily_totals['OrderQty'] >= min_qty)
            if max_qty is None
            else ((df_daily_totals['OrderQty'] >= min_qty) & (df_daily_totals['OrderQty'] <= max_qty))
        )
        historical_qty_values = df_daily_totals.loc[historical_range_mask, 'OrderQty'].astype(int).tolist()
        historical_offer_qty_ranges.append({
            'min_qty': min_qty,
            'max_qty': max_qty,
            'historical_qty_values': historical_qty_values,
            'historical_count': len(historical_qty_values),
            'historical_has_range': len(historical_qty_values) > 0,
        })

    missing_offer_qty_ranges = [
        qty_range for qty_range in historical_offer_qty_ranges
        if not qty_range['historical_has_range']
    ]
    print(f"\n✓ Estratte {len(df_special_offers)} righe da project.specialOffer_prj")
    if missing_offer_qty_ranges:
        print(
            "✓ Alcune fasce sconto non sono presenti nello storico OrderQty: "
            f"{missing_offer_qty_ranges}. Verranno campionate periodicamente."
        )

    print(f"\n✓ Media giornaliera storica di OrderQty: {historical_mean_daily_total:.2f} (std {std_daily_total:.2f})")
    print(f"✓ Media giornaliera target simulazione: {mean_daily_total:.2f}")
    ...
    def genera_vendite_daily_temp(
        ...
        def sample_special_offer_qty(max_qty):
            if max_qty <= 0 or not historical_offer_qty_ranges:
                return None

            candidate_ranges = [
                qty_range for qty_range in historical_offer_qty_ranges
                if qty_range['min_qty'] <= max_qty
            ]
            if not candidate_ranges:
                return None

            weights = np.array([
                max(1, qty_range['historical_count']) * (1.8 if qty_range['min_qty'] >= 11 else 1.0)
                for qty_range in candidate_ranges
            ], dtype=float)
            weights = weights / weights.sum()
            qty_range = candidate_ranges[int(np.random.choice(len(candidate_ranges), p=weights))]
            valid_historical_values = [
                qty for qty in qty_range['historical_qty_values']
                if qty <= max_qty
            ]
            if valid_historical_values:
                return int(np.random.choice(valid_historical_values))
            return int(qty_range['min_qty'])
        ...
        def get_unit_price(product_id):
            ...
            return round(list_price * UNIT_PRICE_MULTIPLIER, 2)
        ...
        for order_index in range(target_orders):
            ...
            forced_offer_qty = None
            if np.random.rand() < 0.15:
                max_forced_qty = max(1, remaining_total - (n_lines - 1))
                forced_offer_qty = sample_special_offer_qty(max_forced_qty)
                if forced_offer_qty is not None:
                    order_target = max(order_target, forced_offer_qty + n_lines - 1)
                    order_target = min(order_target, remaining_total)
            ...
            for line_index in range(n_lines):
                ...
                if forced_offer_qty is not None and line_index == 0:
                    order_qty = min(forced_offer_qty, max_qty)
                ...
                row = {
                    ...
                    'LineTotal': round(order_qty * unit_price, 2)
                }
        ...
        if current_total < target_total:
            while current_total < target_total:
                ...
                remaining_total = target_total - current_total
                if np.random.rand() < 0.15:
                    order_qty = sample_special_offer_qty(remaining_total)
                else:
                    order_qty = None
        ...
        if not df_new.empty:
            def match_special_offer(order_qty):
                qty = int(order_qty)
                matched = df_special_offers[
                    (df_special_offers['MinQty'] <= qty)
                    & (
                        df_special_offers['MaxQty'].isna()
                        | (df_special_offers['MaxQty'] >= qty)
                    )
                ]
                if matched.empty:
                    no_discount = df_special_offers[df_special_offers['UnitPriceDiscount'] == 0]
                    if not no_discount.empty:
                        return no_discount.iloc[0]
                    return df_special_offers.iloc[-1]
                return matched.iloc[0]

            matched_offers = pd.DataFrame([
                match_special_offer(order_qty).to_dict()
                for order_qty in df_new['OrderQty']
            ])
            df_new['SpecialOfferID'] = matched_offers['SpecialOfferID'].astype(int).values
            df_new['UnitPriceDiscount'] = matched_offers['UnitPriceDiscount'].astype(float).round(4).values
        return df_new
