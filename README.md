...
The automated daily procedure checks whether new unprocessed data is available for the current day and executes the historical load only when required. The Python simulator owns the staging-table lifecycle: it creates the staging tables when needed, clears previous staging rows and inserts the next simulated daily batch.
